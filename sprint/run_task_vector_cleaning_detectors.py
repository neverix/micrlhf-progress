...
for task in tqdm(task_names):
    ...
    runner = ICLRunner(task, pairs, batch_size=batch_size, n_shot=n_shot, max_seq_len=max_seq_len, seed=seed, prompt=prompt, vector_type="detector")
    ...
    for layer in layers:
        ...
        fs = FeatureSearch(task, pairs, layer, llama, tokenizer, n_shot=1, seed=seed+100, init_w=gpr, early_stopping_steps=200, n_first=2, sep=sep, newline=newline, pad_token=0, sae_v=8, sae=sae, batch_size=24, iterations=1000, prompt=prompt, l1_coeff=0.0005, feature_type="detector", lr=0.01)
        ...
        with open("cleanup_results_final_detectors_2.jsonl", "a") as f:
            item = {
                "task": task,
                "weights": w.tolist(),
                "loss": loss.tolist(),
